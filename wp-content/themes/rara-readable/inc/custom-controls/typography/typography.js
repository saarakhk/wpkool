...
wp.customize.controlConstructor['typography'] = wp.customize.Control.extend( {
    ready: function() {
		...
		renderSubControl = function( fontFamily, sub, startValue ) {
			...
			_.each( rara_readable_all_fonts, function( font, i ) {
			...
			if ( true === isStandard ) {
				...
				if ( 2 > subList.length ) {
					...
					control.container.find( '.rara-variant-wrapper' ).css( 'display', 'none' );
				} else {
					control.container.find( '.rara-variant-wrapper' ).css( 'display', 'block' );
				}
		...
		jQuery( fontFamilySelector ).selectize({
			options:     rara_readable_all_fonts,
