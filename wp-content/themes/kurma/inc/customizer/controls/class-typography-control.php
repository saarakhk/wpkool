...
if ( class_exists( 'WP_Customize_Control' ) && ! class_exists( 'Kurma_Typography_Customize_Control' ) ) {
	/**
	 * Create the typography elements control.
	 *
	 */
	class Kurma_Typography_Customize_Control extends WP_Customize_Control {
		public $type = 'kurma-customizer-typography';

		public function enqueue() {
			wp_enqueue_script( 'kurma-typography-selectWoo', trailingslashit( get_template_directory_uri() )  . 'inc/customizer/controls/js/selectWoo.min.js', array( 'customize-controls', 'jquery' ), KURMA_VERSION, true );
			wp_enqueue_style( 'kurma-typography-selectWoo', trailingslashit( get_template_directory_uri() )  . 'inc/customizer/controls/css/selectWoo.min.css', array(), KURMA_VERSION );

			wp_enqueue_script( 'kurma-typography-customizer', trailingslashit( get_template_directory_uri() )  . 'inc/customizer/controls/js/typography-customizer.js', array( 'customize-controls', 'kurma-typography-selectWoo' ), KURMA_VERSION, true );
			wp_enqueue_style( 'kurma-typography-customizer', trailingslashit( get_template_directory_uri() )  . 'inc/customizer/controls/css/typography-customizer.css', array(), KURMA_VERSION );
		}

		public function to_json() {
			parent::to_json();

			$number_of_fonts = apply_filters( 'kurma_number_of_fonts', 200 );
			$this->json[ 'default_fonts_title'] = __( 'System fonts', 'kurma' );
			$this->json[ 'google_fonts_title'] = __( 'Google fonts', 'kurma' );
			$this->json[ 'google_fonts' ] = apply_filters( 'kurma_typography_customize_list', kurma_get_all_google_fonts( $number_of_fonts ) );
			$this->json[ 'default_fonts' ] = kurma_typography_default_fonts();
			$this->json[ 'family_title' ] = esc_html__( 'Font family', 'kurma' );
			$this->json[ 'weight_title' ] = esc_html__( 'Font weight', 'kurma' );
			$this->json[ 'transform_title' ] = esc_html__( 'Text transform', 'kurma' );
			$this->json[ 'category_title' ] = '';
			$this->json[ 'variant_title' ] = esc_html__( 'Variants', 'kurma' );
		...
		public function content_template() {
			...
			<# if ( 'undefined' !== typeof ( data.variant ) ) { #>
				...
				#>
				<div id={{{ data.variant.id }}}" class="kurma-font-variant" data-saved-value="{{ data.variant.value }}">
					<label>
			...
			<# if ( 'undefined' !== typeof ( data.category ) ) { #>
				<div class="kurma-font-category">
					<label>
							<input name="{{{ data.category.id }}}" type="hidden" {{{ data.category.link }}} value="{{{ data.category.value }}}" class="kurma-hidden-input" />
			...
			<# if ( 'undefined' !== typeof ( data.transform ) ) { #>
				<div class="kurma-font-transform">
					<label>
