( function( api ) {
	...
	api.controlConstructor['kurma-customizer-typography'] = api.Control.extend( {
		ready: function() {
			var control = this;

			control.container.on( 'change', '.kurma-font-family select',
				function() {
					...
					setTimeout( function() {
						// Send our request to the kurma_get_all_google_fonts_ajax function
						var response = jQuery.getJSON({
							type: 'POST',
							url: ajaxurl,
							data: {
								action: 'kurma_get_all_google_fonts_ajax',
								kurma_customize_nonce: kurma_customize.nonce
							},
							async: false,
							dataType: 'json',
						});
						...
						if ( id in fonts ) {
							...
							jQuery( '.kurma-font-family select' ).not( _this ).each( function( key, select ) {
								var parent = jQuery( this ).closest( '.kurma-font-family' );

								if ( _value == jQuery( select ).val() && _this.data( 'category' ) !== jQuery( select ).data( 'category' ) ) {
									if ( ! got_variants ) {
										updated_variants = jQuery( parent.next( '.kurma-font-variant' ).find( 'select' ) ).val();
										got_variants = true;
									}
								}
							} );
							...
							_this.closest( '.kurma-font-family' ).next( 'div' ).show();
							...
							_this.closest( '.kurma-font-family' ).next( 'div' ).hide();
			...
			control.container.on( 'change', '.kurma-font-variant select',
				function() {
					var _this = jQuery( this );
					...
					jQuery( '.kurma-font-variant select' ).each( function( key, value ) {
						var this_control = jQuery( this ).closest( 'li' ).attr( 'id' ).replace( 'customize-control-', '' );
						var parent = jQuery( this ).closest( '.kurma-font-variant' );
			...
			control.container.on( 'change', '.kurma-font-category input',
				function() {
					control.settings['category'].set( jQuery( this ).val() );
				}
			);

			control.container.on( 'change', '.kurma-font-weight select',
				function() {
					control.settings['weight'].set( jQuery( this ).val() );
				}
			);

			control.container.on( 'change', '.kurma-font-transform select',
				function() {
...
jQuery( document ).ready( function($) {
	...
	jQuery( '.kurma-font-family select' ).selectWoo();
	jQuery( '.kurma-font-variant' ).each( function( key, value ) {
		var _this = $( this );
	...
	$( ".kurma-font-family" ).each( function( key, value ) {
		var _this = $( this );
		if ( $.inArray( _this.find( 'select' ).val(), typography_defaults ) !== -1 ) {
			_this.next( '.kurma-font-variant' ).hide();
		}
	});
