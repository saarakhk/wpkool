...
add_action( 'after_setup_theme', 'kurma_setup_woocommerce' );
/**
 * Set up WooCommerce
 *
 */
function kurma_setup_woocommerce() {
	...
	add_action( 'woocommerce_sidebar', 'kurma_construct_sidebars' );
}

if ( ! function_exists( 'kurma_woocommerce_start' ) ) {
	add_action( 'woocommerce_before_main_content', 'kurma_woocommerce_start', 10 );
	/**
	 * Add WooCommerce starting wrappers
	 *
	 */
	function kurma_woocommerce_start() { ?>
...
if ( ! function_exists( 'kurma_woocommerce_end' ) ) {
	add_action( 'woocommerce_after_main_content', 'kurma_woocommerce_end', 10 );
	/**
	 * Add WooCommerce ending wrappers
	 *
	 */
	function kurma_woocommerce_end() { ?>
						</div><!-- .entry-content -->
						<?php
						/**
...
if ( ! function_exists( 'kurma_woocommerce_css' ) ) {
	add_action( 'wp_enqueue_scripts', 'kurma_woocommerce_css', 100 );
	/**
	 * Add WooCommerce CSS
	 *
	 */
	function kurma_woocommerce_css() {
		if ( ! class_exists( 'WooCommerce' ) ) {
			return;
		}

		$mobile = apply_filters( 'kurma_mobile_media_query', '(max-width:768px)' );
...
if ( ! function_exists( 'kurma_bbpress_css' ) ) {
	add_action( 'wp_enqueue_scripts', 'kurma_bbpress_css', 100 );
	/**
	 * Add bbPress CSS
	 *
	 */
	function kurma_bbpress_css() {
...
if ( ! function_exists( 'kurma_buddypress_css' ) ) {
	add_action( 'wp_enqueue_scripts', 'kurma_buddypress_css', 100 );
	/**
	 * Add BuddyPress CSS
	 *
	 */
	function kurma_buddypress_css() {
...
if ( ! function_exists( 'kurma_beaver_builder_css' ) ) {
	add_action( 'wp_enqueue_scripts', 'kurma_beaver_builder_css', 100 );
	/**
	 ...
	 *
	 */
	function kurma_beaver_builder_css() {
